...
namespace UnitTests
{
    public class SaleReportLogicTests
    {
        ...
        private readonly DB_API mockDBApi = new DB_API(); 

        [SetUp]
        public void Setup()
        {
            saleReportLogic = new SaleReportLogic(mockDBApi);
        }
        ...
        public async Task GenerateSalesReportAsync_ReturnsValidSalesReport()
        {
            ...
            mockDBApi.ObtainDailySalesAsync = async (_) =>
            { 
                return mockDailySales;
            };

            mockDBApi.ObtainWeeklySalesAsync = async (_) =>
            {
